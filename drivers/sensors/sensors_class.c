...
static ssize_t sensors_uni_test_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct sensors_classdev *sensors_cdev = dev_get_drvdata(dev);
	ssize_t ret = -EINVAL;
	...
	ret = kstrtoul(buf, 10, &data);
	pr_err(":::::::customer::::::::data=%ld:::::::::%s",data,__func__);
	if (ret)
		return ret;
	if (data > 1) {
		dev_err(dev, "Invalid value of input, input=%ld\n", data);
	...
	if (sensors_cdev->sensors_uni_self_test == NULL) {
		dev_err(dev, "Invalid sensor class enable handle\n");
		return -EINVAL;
	}
	ret = sensors_cdev->sensors_uni_self_test(sensors_cdev, data);
	if (ret)
		return ret;

	sensors_cdev->uni_calibrate_enabled = data;
	pr_err(":::::::customer::::::::uni_calibrate_enabled=%ld:::::::::%s",data,__func__);
	return size;
}


static ssize_t sensors_uni_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sensors_classdev *sensors_cdev = dev_get_drvdata(dev);
	pr_err(":::::::customer::::::::uni_calibrate_enabled=%d:::::::::%s",sensors_cdev->uni_calibrate_enabled,__func__);
	
	return snprintf(buf, PAGE_SIZE, "%u\n",
			sensors_cdev->uni_calibrate_enabled);
}
#endif
static struct device_attribute sensors_class_attrs[] = {
	...
	#ifdef FT5X06_TPD_PROXIMITY  //liguowei@uni_drv 20150423 add tp proximity function												//zhoulongqing@uniscope add tp use proximity
	__ATTR(enable, 0666, sensors_enable_show, sensors_enable_store),
	#else
	...
	#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
 	__ATTR(uni_self_test, 0666, sensors_uni_test_show, sensors_uni_test_store),
       #endif
