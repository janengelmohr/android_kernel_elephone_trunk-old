...
static int mmc_gpio_get_status(struct mmc_host *host)
{
	...
	if (!ctx || !gpio_is_valid(ctx->cd_gpio))
		goto out;
#if defined UNISCOPE_DRIVER_L701 ||defined UNISCOPE_DRIVER_L702 //liguowei@uni 20150121 add SD hot plug function :GPIO38 high--remove; low--inserted; 
	ret = !(!gpio_get_value_cansleep(ctx->cd_gpio)) ^
		!!(host->caps2 & MMC_CAP2_CD_ACTIVE_HIGH);
#else //(qualcomm default:GPIO38: low--remove; high-inserted)
	ret = !gpio_get_value_cansleep(ctx->cd_gpio) ^
		!!(host->caps2 & MMC_CAP2_CD_ACTIVE_HIGH);
#endif
out:
