...
void mmc_power_up(struct mmc_host *host)
{
	 ...
	 */
	mmc_delay(10);
#ifdef UNISCOPE_DRIVER_DTV //liguowei@uniscope 20140723 support siano dtv
	/* Set signal voltage to 1.8V */
...
struct work_struct *rescanwork=NULL;

void mmc_set_rescan_work(struct work_struct *work)
{
	rescanwork = work;
}

void mmc_rescan_mmc1(void)
{
	struct mmc_host *host =
		container_of(rescanwork, struct mmc_host, detect.work);
	pr_err("mmc_rescan_mmc1");


	if(!rescanwork)
		pr_err("zsf mmc_rescan_mmc1 no rescanwork");
...
//	mmc_detect_change(host, 0);
	...
	mmc_rescan(rescanwork);
}

void mmc_poweroff_mmc1(void)
{
	struct mmc_host *host =
		container_of(rescanwork, struct mmc_host, detect.work);

	
	pr_err("mmc_poweroff_mmc1");
	if(!rescanwork)
		pr_err("zsf mmc_poweroff_mmc1 no rescanwork");
...
//	mmc_detect_change(host, 0);
	...
	//mmc_detect_card_removed(host);
	mmc_suspend_host(host);

	//mmc_rescan(rescanwork);
}
//uniscope@zhongxiao 20140818 add for suspend and resume call at same time 
int sms_sdio_ctrl (unsigned int sdio_port_num, unsigned int on)
{
    ...
    if (!sms_sdio_status && on) {
		/* on -> off */
//		printk(KERN_INFO "%s:force [sdio%d] off before on\n", __func__ ,sdio_port_num);
        mmc_poweroff_mmc1();
        sms_sdio_status = 0;
        /* off -> on */
        mmc_rescan_mmc1();
...
//uniscope@zhongxiao 20140818 add end
#endif
...
#ifdef UNISCOPE_DRIVER_DTV //liguowei@uniscope 20140722 support siano dtv
	pr_err("zsf mmc_rescan %d,%d",host->rescan_disable,host->caps);

	if(strcmp(mmc_hostname(host),"mmc1") == 0)
	{
		if(!rescanwork)
		{
			pr_err("zsf mmc_rescan 1");
			mmc_set_rescan_work(work);
		return;
		}
	}

	if (host->rescan_disable){
		pr_err("zsf mmc_rescan 2  host->rescan_disable");
		return;
	}
	/* If there is a non-removable card registered, only scan once */
	if ((host->caps & MMC_CAP_NONREMOVABLE) && host->rescan_entered){
		pr_err("zsf mmc_rescan 3 host->caps & MMC_CAP_NONREMOVABLE");
		return;
	}
#else
	if (host->rescan_disable)
