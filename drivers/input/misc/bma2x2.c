...
#endif
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
struct bma2x2_data *uni_data=NULL;
#endif
...
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
	struct mutex uni_calibrate_enable_mutex;
	#endif
...
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
               .uni_calibrate_enabled= 0,
		#endif
		.sensors_enable = NULL,
		.sensors_poll_delay = NULL,
		.sensors_self_test = NULL,
		#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
		.sensors_uni_self_test = NULL,
		#endif
};
...
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
static ssize_t bma2x2_uniscope_selfcalibration_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char uni_data_x=0,uni_data_y=0,uni_data_z=0;

	 bma2x2_get_offset_x(uni_data->bma2x2_client, &uni_data_x);
	 bma2x2_get_offset_y(uni_data->bma2x2_client, &uni_data_y);
	 bma2x2_get_offset_z(uni_data->bma2x2_client, &uni_data_z);

	return snprintf(buf, PAGE_SIZE, "%d %d %d\n", uni_data_x, uni_data_y,uni_data_z);
}
	...
	unsigned char target_z)
{
	...
	target[1] = target_y;
	target[2] = target_z;
	if (bma2xx_fast_compensate(target) != 0)
	{
		return -2;
	}
	...
	const unsigned char target[3])
{
	...
	res = bma2x2_set_mode(uni_data->bma2x2_client, BMA2X2_MODE_NORMAL);
	...
	/*** set +/-2g range ***/
	res = bma2x2_set_range(uni_data->bma2x2_client,BMA2X2_RANGE_2G);
	if (res != 0)
	{
		return -2;
	}
	...
	res = bma2x2_set_bandwidth(uni_data->bma2x2_client, BMA2X2_BW_1000HZ);
	...
	res = bma2x2_smbus_read_byte(uni_data->bma2x2_client,BMA2XX_OFFSET_PARAMS_REG, &databuf);
	if (res != 0)
	{
		return -4;
	}
	...
	res = bma2x2_smbus_write_byte(uni_data->bma2x2_client,BMA2XX_OFFSET_PARAMS_REG, &databuf);
	if (res != 0)
	{
		return -5;
	}
	...
	res = bma2x2_smbus_read_byte(uni_data->bma2x2_client,BMA2XX_OFFSET_CTRL_REG, &databuf);
	if (res != 0)
	{
		return -6;
	}
	...
	databuf |= 0x01 << 5;
	res = bma2x2_smbus_write_byte(uni_data->bma2x2_client,BMA2XX_OFFSET_CTRL_REG, &databuf);
	if (res != 0)
	{
		return -7;
	}
	...
	{
		mdelay(2);
		bma2x2_smbus_read_byte(uni_data->bma2x2_client,BMA2XX_OFFSET_CTRL_REG, &databuf);
...
static int bma2xx_eeprom_prog(void)
{
	...
	res = bma2x2_smbus_read_byte(uni_data->bma2x2_client,BMA2XX_EEPROM_CTRL_REG, &databuf);
	if (res != 0)
	{
		return -1;
	}
	databuf |= 0x01;
	res = bma2x2_smbus_write_byte(uni_data->bma2x2_client,BMA2XX_EEPROM_CTRL_REG, &databuf);
	if (res != 0)
	{
		return -2;
	}
	...
	{
		mdelay(2);
		bma2x2_smbus_read_byte(uni_data->bma2x2_client,BMA2XX_EEPROM_CTRL_REG, &databuf);
...
static ssize_t bma2x2_uniscope_selfcalibration_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	unsigned long  val = 0;
	int rc =0;
	static int uniscope_calibration_flag=0;
	unsigned char uni_data_x=0,uni_data_y=0,uni_data_z=0;
	
	rc = kstrtoul(buf, 10, &val);
	if (rc != 0)
		return rc;
		pr_err("::::::::uniscope::::::val=%lu   ::::: \n",val);
	if(val==1&&uniscope_calibration_flag<1)
		{		
		 bma2x2_get_offset_x(uni_data->bma2x2_client, &uni_data_x);
		 bma2x2_get_offset_y(uni_data->bma2x2_client, &uni_data_y);
		 bma2x2_get_offset_z(uni_data->bma2x2_client, &uni_data_z);
		pr_err("::::::::uniscope::::::uni_data_x=%d   uni_data_y=%d   uni_data_z=%d::::: \n",uni_data_x,uni_data_y,uni_data_z);
               if(1)//uni_data_x==0 && uni_data_y==0 && uni_data_z==0) //liguowei@uni uniscope can calibrate many times( <8 )
               	{
		bma2xx_offset_fast_cali(0,0,2);
		uniscope_calibration_flag++;
			pr_err("::::::::uniscope::::::uniscope_calibration_flag=%d ::::::%s LINE = %d \n",uniscope_calibration_flag,__func__,__LINE__);
               	}
		}
	if(val==0)  
		uniscope_calibration_flag=0;
	return count;
}
#endif
...
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
static void bma2x2_uni_calibrate_set_enable(struct device *dev, int enable)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
	int pre_enable = atomic_read(&bma2x2->uni_calibrate_enable);
	static int uniscope_calibration_flag=0;
	unsigned char uni_data_x=0,uni_data_y=0,uni_data_z=0;
	
	mutex_lock(&bma2x2->uni_calibrate_enable_mutex);
	pr_err("::::::::customer::000::::enable=%d   ::::: \n",enable);
	if (enable) {
		if (pre_enable == 0) {
	if(uniscope_calibration_flag<1)
		{		
		 bma2x2_get_offset_x(uni_data->bma2x2_client, &uni_data_x);
		 bma2x2_get_offset_y(uni_data->bma2x2_client, &uni_data_y);
		 bma2x2_get_offset_z(uni_data->bma2x2_client, &uni_data_z);
		pr_err("::::::::customer:::111:::uni_data_x=%d   uni_data_y=%d   uni_data_z=%d::::: \n",uni_data_x,uni_data_y,uni_data_z);
               if(uni_data_x==0 && uni_data_y==0 && uni_data_z==0)
               	{
		          if(bma2xx_offset_fast_cali(0,0,2)<0)
				  	                      goto mutex_exit;

		       uniscope_calibration_flag++;
			pr_err("::::::::customer:::222:::uniscope_calibration_flag=%d ::::::%s LINE = %d \n",uniscope_calibration_flag,__func__,__LINE__);
               	}
		}
		atomic_set(&bma2x2->uni_calibrate_enable, 1);
		}

	} else {
		if (pre_enable == 1) {
                   uniscope_calibration_flag=0;
	           pr_err("::::::::customer::333::::uniscope_calibration_flag=%d   ::::: \n",uniscope_calibration_flag);  
			atomic_set(&bma2x2->uni_calibrate_enable, 0);

		}
	}
mutex_exit:
	mutex_unlock(&bma2x2->uni_calibrate_enable_mutex);

}
static int bma2x2_uni_self_calibration_xyz(struct sensors_classdev *sensors_cdev,unsigned int uni_enable)
{
	struct bma2x2_data *data = container_of(sensors_cdev,
					struct bma2x2_data, cdev);

	bma2x2_uni_calibrate_set_enable(&data->bma2x2_client->dev, uni_enable);
	return 0;
}
#endif
static int bma2x2_eeprom_prog(struct i2c_client *client)
...
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
static DEVICE_ATTR(uniscope_selfcalibration, S_IRUSR|S_IRGRP|S_IWUSR,
		bma2x2_uniscope_selfcalibration_show, bma2x2_uniscope_selfcalibration_store);
#endif
...
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
	&dev_attr_uniscope_selfcalibration.attr,
#endif
		...
		const struct i2c_device_id *id)
{
	...
	#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
	mutex_init(&data->uni_calibrate_enable_mutex);
	#endif
...
#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
	atomic_set(&data->uni_calibrate_enable, 0);
#endif
	...
	#ifdef UNISCOPE_DRIVER_BMA2X2_EEPROM_CALIBRATION //liguowei 20141128 bma2x2 eeprom calibration function
	data->cdev.sensors_uni_self_test = bma2x2_uni_self_calibration_xyz;
	#endif
