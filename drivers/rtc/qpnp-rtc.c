...
/* (JZZ)Zhizhang@uniscope_drv for sometimes RTC power on automatic
   when attach battery after remove battery 20140711 */	
#if defined(UNISCOPE_DRIVER_DEBUG_AUTORTC_POWERON)
extern void backup_alarm(u8 val);
#endif
...
/* (JZZ)Zhizhang@uniscope_drv for sometimes RTC power on automatic
   when attach battery after remove battery 20140711 */	
#if defined(UNISCOPE_DRIVER_DEBUG_AUTORTC_POWERON)
	//dev_err(dev, "qpnp_rtc_set_alarm JZZ temp %d, alarm->enabled %d.rtc_dd->alarm_ctrl_reg1 %d.\n",temp,alarm->enabled,rtc_dd->alarm_ctrl_reg1);	
	
	if(alarm->enabled) 
	{ 
		//dev_err(dev, "Alarm enabled,will backup"); 
		temp=value[0];//|value[1]|value[2]|value[3];//add your theory here 
		backup_alarm(temp);
	} 
	else
	{
...
/* (JZZ)Zhizhang@uniscope_drv for sometimes RTC power on automatic
   when attach battery after remove battery 20141118 */	
#if defined(UNISCOPE_DRIVER_DEBUG_AUTORTC_POWERON)
	dev_err(rtc_dd->rtc_dev, "qpnp_rtc_shutdown JZZ rtc_dd->alarm_ctrl_reg1 %d.\n",rtc_dd->alarm_ctrl_reg1);	
	reg = rtc_dd->alarm_ctrl_reg1;	
	if((reg >> 7) == 0)
	{
		dev_err(rtc_dd->rtc_dev, "qpnp_rtc_shutdown JZZ5 rtc_dd->alarm_ctrl_reg1 %d.\n",rtc_dd->alarm_ctrl_reg1);	
		backup_alarm(0);
	} 
#endif
