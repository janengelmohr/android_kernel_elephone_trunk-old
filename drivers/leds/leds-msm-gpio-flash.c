...
/* JZZ(zhizhang)@uniscope_drv for adding front 
	camera flashlight function 20140804 */
#if defined(UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT)
#define LED_GPIO_FLASH_SUB_DRIVER_NAME  "qcom,leds-gpio-flash_sub"
#endif
...
/* JZZ(zhizhang)@uniscope_drv for adding front 
	camera flashlight function 20140804 */
#if defined(UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT)	
	{.compatible = LED_GPIO_FLASH_SUB_DRIVER_NAME,},
#endif	
	{},
};
...
int led_gpio_flash_probe(struct platform_device *pdev)
{
	...
	flash_led->gpio_state_default = pinctrl_lookup_state(flash_led->pinctrl,
		"cam_flash_default");//"ocp8110_default" modify by uni@fantian 20140606
...
#if defined(UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT)	
		if(flash_led->flash_now == -ENOENT)
		{
			dev_err(&pdev->dev,"%s: there is no flash-now configure.\n",__func__);
		}
		else
