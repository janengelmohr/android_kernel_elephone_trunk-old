...
#ifdef UNISCOPE_DRIVER_QC8916
       btc_cfg=0x82;   //liguowei@uniscope.com 20141024 stop charging when the temp upto 60;
       pr_err("uniscope BTC configuration mask=%x\n", btc_cfg);
#endif
...
#ifdef UNISCOPE_DRIVER_L700//liguowei 20141126
extern int uniscope_imobile_charger_flag;
#endif
	...
	if (chip->bms_psy) {
		...
		if (soc == 0) {
			if (!qpnp_lbc_is_usb_chg_plugged_in(chip))
             ...
             #ifdef UNISCOPE_DRIVER_L700//liguowei 20141126
			if(uniscope_imobile_charger_flag==1)
			  {
			    if(soc>=70)
			    	{
...
/*zhoulongqing @uniscope 20140721 modify charging current begin*/
...
/*zhoulongqing @uniscope 20140721 modify charging current end*/
