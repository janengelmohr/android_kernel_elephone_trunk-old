...
#ifdef UNISCOPE_DTV_DEBUG //liguowei@uniscope.com 20140801 dtv
int sms_debug = 0;// 3;
#else
int sms_debug = 0;
#endif
...
int smscore_register_device(struct smsdevice_params_t *params, void **coredev)
{
	...
	dev->mode = SMSHOSTLIB_DEVMD_DVBT2;//byj SMSHOSTLIB_DEVMD_NONE;
	dev->board_id =  SMS1XXX_BOARD_SIANO_SIENA;//byj SMS_BOARD_UNKNOWN;
...
#ifndef UNISCOPE_SMS_STATIC_MEM
		sms_static_firmware = kmalloc(ALIGN(fw->size + sizeof(sms_static_firmware), SMS_ALLOC_ALIGNMENT),
				GFP_KERNEL | GFP_DMA);
#else
		sms_static_firmware = kmalloc(ALIGN(sizeof(sms_static_firmware), SMS_ALLOC_ALIGNMENT),
				GFP_KERNEL | GFP_DMA);
		memset (pdata, 0, sizeof(pdata));
		sms_static_firmware->data = (void*)pdata;
#endif
		if (!sms_static_firmware) {
...
//uniscope@zhongxiao 20140804 delete for crash			sms_debug("msgType:%d  Length:%d\n", phdr->msgType, phdr->msgLength);
	}
}
...
//	sms_err("smscore_validate_client");
			    ...
			    struct smscore_client_t **client)
{
	...
	//sms_err("smscore_register_client");
...
{
	//sms_err("unregister client 0x%p", client->context);
	struct smscore_device_t *coredev = client->coredev;
	unsigned long flags;
//	sms_err("smscore_unregister_client");
...
{
    sms_err("sms_chip_poweron \n");
#if 1//lgw 	

    if(gpio_is_valid(971)) //dtv detect  gpio69
    {
    gpio_set_value(971,1);
    sms_err("dtv detect  gpio69 --high\n");
    }
    msleep(10);  

    if(gpio_is_valid(1020)) //dtv reset gpio118
    {
        sms_err("dtv reset gpio118  --low \n");
        gpio_set_value(1020,0);
    }
    msleep(10);

    if(gpio_is_valid(938)) //vcc_1.8 gpio36
    {
	    gpio_set_value(938,1);
	    sms_err("vcc_1.8 gpio36 --high\n");
    }
    msleep(10);  //siano 0-30ms
    
    if(gpio_is_valid(953)) //vcc_1.2 gpio51
    {
	    gpio_set_value(953,1);
	    sms_err("vcc_1.2 gpio51 --high\n");
    }
    msleep(10); //siano 0-15ms
   
    if(gpio_is_valid(954)) //vccio_1.8 gpio 52
    {
	    gpio_set_value(954,1);
	    sms_err("vccio_1.8 gpio52 --high\n");
    }
    msleep(100);
    
    if(gpio_is_valid(1020)) //dtv reset gpio118
    {
     	sms_err("dtv reset gpio118  --high \n");
        gpio_set_value(1020,1);
    }

    msleep(200);//msleep(500);//uniscope@zhongxiao 20140818 modify for dtv open time saving  
    sms_err("sms_chip_poweron end\n");
#endif 
}
...
static void sms_chip_poweroff(void)
{
    ...
    if(gpio_is_valid(954)) //vccio_1.8 gpio 52
	{
    	...
    	sms_err("vccio_1.8 gpio 52 --low\n");
	    msleep(10);  //siano 0-30ms
	}

    if(gpio_is_valid(953)) //vcc_1.2 gpio51
	{
		gpio_set_value(953,0);
    	sms_err("vcc_1.2 gpio51 --low\n");
	    msleep(10); //siano 0-15ms
	}

    if(gpio_is_valid(938)) //vcc_1.8 gpio36
	{
		gpio_set_value(938,0);
    	sms_err("vcc_1.8 gpio36 --low\n");
	    msleep(10);//msleep(500);//uniscope@zhongxiao 20140818 modify for dtv open time saving  
    }
     sms_err("sms_chip_poweroff end");  
#endif
}


extern void test_mmc_rescan(void);
extern void mmc_rescan_mmc1(void);
extern void mmc_poweroff_mmc1(void);
...
{
#if 1 //byj 	
    sms_err("sms_chip_plug 11 plugin=%d\n", plugin);
	if (plugin) {
    	sms_chip_poweron();
#ifdef SMS_SDIO_DRV
		sms_sdio_plug_notify(plugin);//uniscope@zhongxiao 20140818 modify to notify  for suspend and resume call at same time 
		msleep(50);//msleep(500);//uniscope@zhongxiao 20140818 modify for dtv open time saving  
#endif
	} else {
#ifdef SMS_SDIO_DRV
		sms_sdio_plug_notify(plugin);//uniscope@zhongxiao 20140818 modify to notify  for suspend and resume call at same time 
		msleep(50);//msleep(1000);//uniscope@zhongxiao 20140818 modify for dtv open time saving  
#endif
		sms_chip_poweroff();
	}
#endif
...
#ifdef SMS_POWER_CTL_INSMOD
//	sms_err("xx 44 call sms_chip_plug  0\n");
...
#ifdef SMS_POWER_CTL_INSMOD
//		sms_err("xx 55 call sms_chip_plug  0\n");
