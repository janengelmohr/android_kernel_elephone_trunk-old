...
int smschar_send_fw_file(struct smschar_device_t *dev,
	...
	sms_static_firmware = kmalloc(ALIGN(up->fw_size + sizeof(sms_static_firmware), SMS_ALLOC_ALIGNMENT), GFP_KERNEL
			| GFP_DMA);
#else
	sms_static_firmware = kmalloc(ALIGN(sizeof(sms_static_firmware), SMS_ALLOC_ALIGNMENT), GFP_KERNEL
			| GFP_DMA);
...
{
	unsigned long flags;
//	sms_err("smschar_unregister_client");
	...
	if (dev->smsclient){
//		sms_err("smschar_unregister_client   dev->smsclient");
...
//	    sms_err("smschar_open 11\n");
		...
		if ((OPEN_COUNT_INIT + 1) == atomic_inc_return(&open_count)) {
			//sms_err("smschar_open open_count:%d, dev->coredev:%#x\n", atomic_read(&open_count), (unsigned int)(dev->coredev));
			if (!(dev->coredev)) {
				sms_err("111 uniscope init for one time");
				sms_chip_plug(1); //liguowei@uni add poweron one times
				if (!dev->coredev) {
					while (times-- && !dev->coredev){
						msleep(50);
					};

                            if (!dev->coredev) //liguowei@uni add poweron two times
                            	{
				sms_err("222 uniscope init for two time");
                            	       times = 100;
						sms_chip_plug(0);
						msleep(50);
				              sms_chip_plug(1);
						while (times-- && !dev->coredev){
                            ...
                            if (!dev->coredev) //liguowei@uni add poweron three times
                            	{
				sms_err("333 uniscope init for three time");
                            	       times = 100;
						sms_chip_plug(0);
						msleep(50);
				              sms_chip_plug(1);
						while (times-- && !dev->coredev){
                            ...
                            if (!dev->coredev) //liguowei@uni add poweron four times
                            	{
				sms_err("444 uniscope init for four time");
                            	       times = 100;
						sms_chip_plug(0);
						msleep(50);
				              sms_chip_plug(1);
					...
					if (!dev->coredev) {
						...
						//sms_err("xx 22 call sms_chip_plug  0\n");
...
static int smschar_release(struct inode *inode, struct file *file)
{
	...
	if (dev->cdev.dev == MKDEV(smschar_major, smschar_minor)) { //not all device needed, just "mdtvctrl"
		...
		if (OPEN_COUNT_INIT == atomic_dec_return(&open_count)) {
			...
			//sms_err("xx 33 call sms_chip_plug  0\n");
...
//	sms_err("smschar_read count=%d", count);
			     ...
			     size_t count, loff_t *f_pos)
{
	...
	//sms_err("smschar_write count=%d, buf=%s\n", count, buf);
...
//	sms_err("smschar_wait_get_buffer\n");
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	//sms_err("smschar_ioctl   cmd:0x%x\n", cmd);
	switch (cmd) {
...
static int smschar_power_mode_handler(enum sms_power_mode_st mode)
{
	...
	//sms_err("smschar_power_mode_handler.\n");
	switch (mode) {
...
//	sms_err("smscore_register_hotplug(smschar_hotplug);");
...
//	sms_err("smscore_unregister_hotplug(smschar_hotplug);");
