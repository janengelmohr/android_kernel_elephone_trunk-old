...
/* JZZ(zhizhang)@uniscope_drv  20140804 */
#if 0//defined(UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT_DEBUG)	
	if(fctrl->pdev->name)
		pr_err("msm_led_trigger_config name %s,called led_state %d,num %d.subdev_id %d, posion %d.\n", fctrl->pdev->name, cfg->cfgtype,fctrl->num_sources,fctrl->subdev_id,cfg->position);

	if(!strncmp(fctrl->pdev->name, "qcom,camera-led-flash" ,20))
		pr_err("msm_led_trigger_config fctrl->pdev->name = qcom,camera-led-flash");		

	if(fctrl->pdev->id_entry && fctrl->pdev->id_entry->name)
		pr_err("msm_led_trigger_config platform_device id_entry name %s.autoid %d,",fctrl->pdev->id_entry->name,(int)fctrl->pdev->id_entry->driver_data);

	if(fctrl->flashdata)
	{
		if(fctrl->flashdata->sensor_name)
			pr_err("msm_led_trigger_config flashdata name %s.\n",fctrl->flashdata->sensor_name);

		if(fctrl->flashdata->slave_info && fctrl->flashdata->sensor_info)
			pr_err("msm_led_trigger_config flashdata sensor_id %d. session_id %d\n",fctrl->flashdata->slave_info->sensor_id,fctrl->flashdata->sensor_info->session_id);
	}
#endif /*UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT_DEBUG*/
	switch (cfg->cfgtype) {
	case MSM_CAMERA_LED_OFF:
/* JZZ(zhizhang)@uniscope_drv for adding front camera flashlight function 20140804 */
#if 0//defined(UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT)		
		if(fctrl->pdev->name && !strncmp(fctrl->pdev->name, "qcom,camera-led-flash" ,20))
		{
			for (i = 0; i < fctrl->num_sources; i++)
				if (fctrl->flash_trigger[i])
					dual_led_trigger_event(fctrl->flash_trigger[i], 0,cfg->position);
			if (fctrl->torch_trigger)
				dual_led_trigger_event(fctrl->torch_trigger, 0,cfg->position);		
		}
		else
#endif /*UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT*/
               {
		/* Flash off */
...
/* JZZ(zhizhang)@uniscope_drv for adding front and rear camera flashlight function 20140804 */
#if 0//defined(UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT)			
			if(fctrl->pdev->name && !strncmp(fctrl->pdev->name, "qcom,camera-led-flash" ,20))
				dual_led_trigger_event(fctrl->torch_trigger,
					curr_l,cfg->position);
			else	
#endif	/*UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT*/
...
#if 0//defined(UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT)				
				if(fctrl->pdev->name && !strncmp(fctrl->pdev->name, "qcom,camera-led-flash" ,20))
					dual_led_trigger_event(fctrl->flash_trigger[i],
						curr_l,cfg->position);
				else	
#endif /*UNISCOPE_DRIVER_FRONT_CAMERA_FLASHLIGHT*/
