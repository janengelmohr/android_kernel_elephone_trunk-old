...
*/
	{0xfe,0x00},//default
	{0xbf,0x08},
#if defined(UNISCOPE_DRIVER_L500)
	{0xc0,0x16},
	{0xc1,0x28},
	{0xc2,0x41},
	{0xc3,0x5a},
...
#if defined(UNISCOPE_DRIVER_L500)
	{{0xfe, 0x01},{0x13, 0x38},{0xfe, 0x00}},//Exposure//38
#else
	{{0xfe, 0x01},{0x13, 0x44},{0xfe, 0x00}},//Exposure//38
#endif
	{{0xfe, 0x01},{0x13, 0x4c},{0xfe, 0x00}},//Exposure +1
...
{
	printk(" wdp %s %d\n", __func__, value);
	   printk(" wdp gc0310_PETER£¬gc0310_set_antibanding = %x" , value);
	...
	void __user *argp)
{
	...
	case CFG_SET_RESOLUTION:  //peter
	{
		...
		if (copy_from_user(&val,
			...
			break;
	    	   ...
	    	   printk(" wdp gc0310_PETER-preview/capture-VAL = %d" , val);
	...
	case CFG_SET_SLAVE_INFO: {
		...
		for (slave_index = 0; slave_index <
			...
			printk(" wdp %s i %d power setting %d %d %ld %d\n", __func__,
				slave_index,
	...
	case CFG_SET_ANTIBANDING: {
		...
		printk(" wdp %s: anti-banding mode is %d\n", __func__,
			antibanding_mode);
	...
	case CFG_SET_BESTSHOT_MODE: {
		...
		printk(" wdp %s: best shot mode is %d\n", __func__, bs_mode);
	...
	case CFG_SET_WHITE_BALANCE: {
		...
		printk(" wdp %s: white balance is %d\n", __func__, wb_mode);
	...
	void __user *argp)
{
	...
	case CFG_SET_RESOLUTION: 
		...
		int32_t val;
		if (copy_from_user(&val,
			(void *)compat_ptr(cdata->cfg.setting), sizeof(int32_t))) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			rc = -EFAULT;
			break;
		}
	...
	case CFG_SET_STREAM_TYPE: {
		enum msm_camera_stream_type_t stream_type = MSM_CAMERA_STREAM_INVALID;
		if (copy_from_user(&stream_type, (void *)compat_ptr(cdata->cfg.setting),
			sizeof(enum msm_camera_stream_type_t))) {
			pr_err("%s:%d failed\n", __func__, __LINE__);
			rc = -EFAULT;
			break;
